...
public class log4jTest {
	...
	public void test() {
		logger.fatal("Fatal로그!"); // 아주 심각한 에러일 경우. log4에서 사용 가능함.
		
		logger.error("Error로그!"); // 요청처리중 에러가 발생한 경우
		
		logger.warn("Warn로그!"); // 프로그램실행에는 당장 문제가 없지만, 문제소지 있음.
		
		logger.info("Info로그!"); // 상태변경과 같은 정보성 메세지
		
		logger.debug("Debug로그!"); // 개발모드
		
		logger.trace("Trace로그!"); // 디버그레벨을 좀 더 세분화해서 관리하기 위한 레벨
